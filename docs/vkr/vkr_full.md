...
Критерий прохождения: в каждом модуле одновременно выполняются пороги `покрытие строк >= 50%` и `покрытие ветвлений >= 40%`. При нарушении порога задача Gradle завершается ошибкой, что обеспечивает применимость проверки в контуре CI.

### 3.8.2 Методика расчета коэффициента покрытия

В работе используется JaCoCo. Для каждого счетчика рассчитывается отношение покрытых элементов к общему числу элементов в расчетном контуре:
...
Исключение из расчета покрытия не означает отсутствия проверки: часть таких компонентов контролируется косвенно через интеграционные тесты и сквозные пользовательские сценарии (раздел 3.7). Метрика покрытия в данном контуре используется именно как индикатор качества автоматизированных тестов бизнес-логики.
...
| Модуль | XML-отчеты тестов | Запущено тестов | Ошибок | Пропусков | Строк в контуре (покрыто/всего) | Покрытие строк | Ветвей в контуре (покрыто/всего) | Покрытие ветвлений | Пороговые значения (строки/ветви) | Пороговый контроль |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
...
В результате автоматизированная проверка дополняет ручную верификацию сквозных сценариев (раздел 3.7) и формирует воспроизводимый контур контроля качества для последующих итераций разработки [9].
