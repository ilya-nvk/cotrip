...
Критерий прохождения: в каждом модуле одновременно выполняются пороги `покрытие строк >= 50%` и `покрытие ветвлений >= 40%`. При нарушении порога задача Gradle завершается ошибкой, что обеспечивает применимость проверки в контуре непрерывной интеграции.
