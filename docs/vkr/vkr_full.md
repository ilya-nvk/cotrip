...
- **ТП** — текст программы.
- **Deep link** — ссылка, открывающая конкретный экран мобильного приложения.
- **CI (Continuous Integration)** — практика непрерывной интеграции изменений в общий репозиторий с автоматическим запуском проверок сборки и тестов.
...
Примечание: в матрице используются только бинарные оценки. Если функция присутствует фрагментарно и не закрывает критерий целиком, фиксируется значение «Нет».
...
| Идея -> обсуждение -> маршрут | Подсистема идей, новые комментарии real-time, модуль маршрута | Предложение участника после обсуждения фиксируется в плане дня | Конкурирующие изменения статуса идеи |
...
**[МЕСТО ДЛЯ РИСУНКА 2.1: ключевой пользовательский поток "идея -> обсуждение -> маршрут" (вставляется вручную)]**
Подпись к вставке: схема потока из 3-5 блоков (`идея -> обсуждение -> добавление в маршрут`), источник — текущая версия приложения; рисунок должен подтверждать связность шагов пользовательского сценария.
...
**[МЕСТО ДЛЯ РИСУНКА 2.2: обзорная архитектура системы (экспорт схемы/скрин архитектурной диаграммы) (вставляется вручную)]**
Подпись к вставке: архитектурная диаграмма `Android client -> Backend -> PostgreSQL/интеграции`, источник — проектная схема; рисунок должен подтверждать границы подсистем и направления обмена данными.
...
Клиент построен по многоуровневой схеме `UI -> ViewModel -> Repository -> Data source`.
...
3. Репозитории инкапсулируют работу с REST API, локальным кэшем и очередью синхронизации, чтобы экран не зависел от источника данных напрямую.
4. Инфраструктурный слой включает внедрение зависимостей (Hilt) и фоновые задачи синхронизации (WorkManager), которые восстанавливают согласованность после появления сети.
...
Сервер выступает источником истинного состояния, а клиент поддерживает локальную копию данных и очередь изменений. При нестабильной сети пользователь продолжает работу с локальными данными, после восстановления связи накопленные операции отправляются на сервер и состояние приводится к согласованному виду [12][13][19].
...
| Поездки и участники | Создание поездки, редактирование и удаление владельцем, приглашения, выход участника из поездки | Быстрое формирование совместного пространства | Валидация дат и проверка корректности приглашений |
...
Для транзакционного хранения предметных сущностей используется PostgreSQL [10][11]. Выбор обусловлен:
1. реляционной моделью с явными связями между поездками, участниками, идеями, маршрутом и расходами;
2. транзакционными гарантиями ACID для операций, влияющих на совместное состояние группы;
...
В клиентской архитектуре зафиксирована связка паттернов `MVVM + Repository + DI`:
1. MVVM изолирует состояние экрана в `ViewModel` и делает поведение экрана воспроизводимым;
2. Repository скрывает различия между сетевым источником, локальным кэшем и отложенной синхронизацией;
3. DI (Hilt) снижает связанность модулей и упрощает модульное тестирование.
...
1. сценарии выполняются в нестабильной сети, поэтому требуется раздельная обработка онлайн- и офлайн-путей (локальный кэш + очередь + фоновая синхронизация);
...
1. **Согласованность данных.** Для поддерживаемых операций сервер задает единые правила валидации и хранения, чтобы участники поездки получали согласованное состояние после синхронизации изменений [12][13].
...
Подпись к вставке: схема/скрин сценария `действие офлайн -> запись в очередь -> восстановление сети -> синхронизация`, источник — экран приложения или схема из проекта; рисунок должен подтверждать непрерывность сценария при потере сети.
...
Подпись к вставке: набор основных экранов приложения (список поездок, детали поездки, идеи, маршрут, расходы), источник — актуальная сборка; рисунок должен подтверждать целостность интерфейса.

В качестве демонстрационного сценария рассматривается совместное планирование поездки, где владелец создает поездку, приглашает участника, формирует идеи и маршрут, распределяет расходы, использует погодные данные и AI-подсказки, а также продолжает работу при временной потере сети. Такой выбор позволяет проверить не отдельные экраны, а целостность системы и связность реализованных модулей [7][8][9].

## 3.1 Реализация подсистемы авторизации и профиля

### Реализация на клиенте
На стороне Android реализован экран входа и связанный поток инициализации сессии. Пользователь проходит авторизацию и после успешного входа попадает в рабочий режим приложения. В клиентской части также реализован экран настроек, где доступны изменение имени, фотография профиля, завершение сессии и удаление учетной записи в предусмотренном пользовательском сценарии [7][9][14].
В интерфейсе применена политика локализации: при системной локали `ru` приложение отображает русский UI, в остальных локалях — английский UI.
...
Подпись к вставке: коллаж `список идей -> обсуждение -> добавление в маршрут`; рисунок должен подтверждать сквозной пользовательский поток.
...
Подпись к вставке: коллаж `действие без сети` и `результат после восстановления сети`; рисунок должен подтверждать применение офлайн-очереди.
...
| Поездки и участники | Создание поездки, редактирование/удаление владельцем, приглашение, присоединение и выход участника | Android + Backend | Реализовано |
...
Проверка выполнялась как ручной сценарный прогон в тестовой среде приложения по набору тестовых сценариев. Представленные результаты отражают выполнение ключевых сценариев и не охватывают все возможные комбинации входных данных [7][8][9].

Проведенная проверка показывает, что реализованная система покрывает основной набор пользовательских сценариев, сформулированный в ТЗ и обоснованный в аналитической части работы [7][8]. При этом дальнейшим этапом развития остается расширение автоматизированного набора регрессионных тестов и углубление проверки граничных случаев.

## 3.8 Автоматизированное тестирование и анализ покрытия

Раздел 3.8 описывает инженерный контроль качества разработки (Android + backend) и не заменяет клиентские приемочные испытания ПМИ. ПМИ фиксирует только пользовательские сценарии клиентского приложения.
...
### 3.8.1 Пошаговый запуск проверок (для внешнего проверяющего)

Чтобы воспроизвести результаты без знания внутренней структуры проекта, используется следующий порядок:
1. открыть корневую директорию репозитория, где находятся каталоги `android/`, `backend/` и скрипт `quality-check.sh`;
...
Сценарий `qualityCheck` в каждом модуле включает:
1. выполнение тестов;
2. формирование отчета JaCoCo в форматах `XML` и `HTML`;
...
`android/app/build/reports/jacoco/jacocoDebugUnitTestReport/html/index.html`.

Критерий прохождения: в каждом модуле одновременно выполняются пороги `покрытие строк >= 50%` и `покрытие ветвлений >= 40%`. При нарушении порога задача Gradle завершается ошибкой, что позволяет применять эту проверку в CI.

### 3.8.2 Методика расчета коэффициента покрытия

В работе используется JaCoCo. Для каждого счетчика рассчитывается отношение покрытых элементов к общему числу элементов в расчетной части кода:

`КоэффициентПокрытия = Покрыто / (Покрыто + Непокрыто) * 100%`

Для ключевых метрик:
1. `ПокрытиеСтрок = ПокрытыеСтроки / (ПокрытыеСтроки + НепокрытыеСтроки) * 100%`
2. `ПокрытиеВетвлений = ПокрытыеВетви / (ПокрытыеВетви + НепокрытыеВетви) * 100%`

Где:
1. `ПокрытыеСтроки` — строки, для которых в ходе теста выполнена хотя бы одна инструкция;
2. `НепокрытыеСтроки` — строки, для которых не выполнено ни одной инструкции;
3. `ПокрытыеВетви/НепокрытыеВетви` — покрытые/непокрытые ветви условных конструкций.

### 3.8.3 Контур файлов и причины исключений из расчета покрытия

Покрытие считается не по всему бинарному артефакту, а по целевой части бизнес-логики, заданной в Gradle-конфигурации [9]. Такой подход нужен, чтобы метрика отражала качество тестирования бизнес-логики, а не технической обвязки.

Для клиентского модуля Android в расчет включаются скомпилированные классы `nvk/cotrip/**` (сборка `debug`), при этом исключаются:
1. сгенерированные и инфраструктурные классы (`R`, `BuildConfig`, `Manifest`, DI/Hilt/Dagger, `*_Factory`, `*_MembersInjector`, `*_Impl`);
2. обвязка пользовательского интерфейса и точки входа (`*ScreenKt*`, `MainActivity`, `CoTripApp`);
3. отдельные инфраструктурные директории (`di/**`, `ui/theme/**`, `data/network/dto/**`);
4. тестовые и синтетические классы (`*Test*`, часть `*$*` шаблонов).

Для серверного модуля покрытие считается по классам основного набора (`main`) с исключением инфраструктурных слоев:
1. точки входа и загрузочная инициализация (`ApplicationKt`, часть подсистемы журналирования и конфигурации);
2. маршруты и транспортный слой (`routes/**`, `plugins/**`);
3. инфраструктура БД (`db/**`, `DatabaseFactory`);
4. часть внешних адаптеров доставки событий (`FirebasePushService`, `CommentEventsPublisher`, `CommentsHub`);
5. служебные/сгенерированные вспомогательные конструкции Kotlin (`$Companion`, `$serializer`, `$WhenMappings`, `$inlined$`).
...
### 3.8.4 Фактические результаты тестирования и покрытия

Количественные показатели получены по результатам запуска `./quality-check.sh` от `18.03.2026` в текущем состоянии репозитория.

| Модуль | XML-отчеты тестов | Запущено тестов | Ошибок | Пропусков | Строк в расчетной части (покрыто/всего) | Покрытие строк | Ветвей в расчетной части (покрыто/всего) | Покрытие ветвлений | Пороговые значения (строки/ветви) | Пороговый контроль |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| Клиент Android (`:app:testDebugUnitTest`) | 61 | 291 | 0 | 0 | 3559 / 5677 | 62.69% | 1597 / 3918 | 40.76% | 50% / 40% | Пройден |
| Сервер (`:test`) | 10 | 46 | 0 | 0 | 534 / 968 | 55.17% | 280 / 577 | 48.53% | 50% / 40% | Пройден |
| Итого по двум модулям (агрегировано по счетчикам JaCoCo) | 71 | 337 | 0 | 0 | 4093 / 6645 | 61.60% | 1877 / 4495 | 41.76% | 50% / 40% | Пройден |

Дополнительно по объектам JaCoCo в расчетной части:
1. клиент Android: 32 пакета, 223 исходных файла, 192 класса в расчетной части (`153` покрыто полностью/частично, `39` не покрыто);
2. сервер: 7 пакетов, 14 исходных файлов, 66 классов в расчетной части (`51` покрыто полностью/частично, `15` не покрыто).

### 3.8.5 Интерпретация полученных значений

Полученные значения демонстрируют, что минимальные пороговые критерии качества выполняются в обоих модулях. При этом:
1. покрытие строк в обоих модулях превышает установленный порог и отражает приемлемый охват исполняемых сценариев;
2. покрытие ветвлений в Android-модуле находится близко к порогу, поэтому набор тестов по альтернативным ветвям условий и граничным состояниям нужно расширять;
...
В результате автоматизированная проверка дополняет ручную верификацию сквозных сценариев (раздел 3.7) и формирует воспроизводимый процесс контроля качества для последующих итераций разработки [9].
...
[11] Инициализационная схема базы данных проекта.
...
## Приложения
### Приложение А. Дополнительные пользовательские сценарии
`[ВСТАВИТЬ МАТЕРИАЛЫ ПРИЛОЖЕНИЯ А]`

### Приложение Б. Дополнительные иллюстрации интерфейса
`[ВСТАВИТЬ МАТЕРИАЛЫ ПРИЛОЖЕНИЯ Б]`

### Приложение В. Дополнительные материалы по тестированию
`[ВСТАВИТЬ МАТЕРИАЛЫ ПРИЛОЖЕНИЯ В]`
